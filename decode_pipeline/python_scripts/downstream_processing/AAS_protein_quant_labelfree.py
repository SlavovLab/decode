...
mtp_bp_pair_dict = {'MTP':'', 'BP':'', 'aa_sub':'', 'aa_sub_idx':'', 'in_tissues':[]}
...
for s in samples:
    ...
    for mtp_idx in list(mtp_dict[s]['Raw file'].keys()):
        ...
        tissues = mtp_quant_dict[mtp_quant_idx]['tissues']
        mtprot_dict[rzr_prot]['MTP-BP_pairs'][-1]['in_tissues'] = tissues
    ...
    for p,pdict in mtprot_dict.items():
        if pdict['Protein_sequence'] == '':
           # if (s in pdict['MTP-BP_pairs'][0]['in_TMT_sets']) and (pdict['Protein_sequence']==''):
            if (s in pdict['MTP-BP_pairs'][0]['in_tissues']) and (pdict['Protein_sequence']==''):
