...
"""
    This code reads in results from DP search with MaxQuant and sample list from label-free analysis
    and outputs dictionaries of data for putative peptides with amino acid substitutions.
    This script is computationally intensive to run and makes use of multiprocessing to split the search for AAS across label-free samples. It is recommended to run in a HPC environment.
    The most intensive part is generating the 6-frame translation of the genome of interest. If run previously for your species, it is recommended to read in those output files and comment out the lines used to generate the translation.
   
"""

# setting directories and reading in data
code_dir = '/home/tsour.s/scripts_templates/AA_subs_pipeline/'
proj_dir = '/scratch/tsour.s/Wang_HealthyTissues/'
aa_subs_dir = proj_dir+'AA_subs_pipeline/'# aa_subs_dir is the output folder

"""read in list of tissues/experiments/samples for which you have independent MaxQuant results"""
tissues = open(aa_subs_dir+'proteomic_tissue_list_pt2.txt', 'r').read().split('\n')
...
def get_data_dict(s):
    ...
    dp_dict['origin aa'] = {i:(aa_subs_dicts[i][0] if aa_subs_dicts[i] else []) for i in aa_subs_dicts.keys()}
    dp_dict['origin aa index'] = {i:(aa_subs_dicts[i][1] if aa_subs_dicts[i] else []) for i in aa_subs_dicts.keys()}
    dp_dict['aa subs'] = {i:(aa_subs_dicts[i][2] if aa_subs_dicts[i] else []) for i in aa_subs_dicts.keys()}
    dp_dict['aa subs positional probability'] = {i:(aa_subs_dicts[i][3] if aa_subs_dicts[i] else []) for i in aa_subs_dicts.keys()}
    dp_dict['aa subs mass error (ppm)'] = {i:(aa_subs_dicts[i][4] if aa_subs_dicts[i] else []) for i in aa_subs_dicts.keys()}
    dp_dict['destination aa'] = {i: [x[-1] for x in v] if len(v)>0 else [] for i,v in dp_dict['aa subs'].items()}
    dp_dict['mistranslated sequence'] = {i:(get_mistranslated_seq(dp_dict, i) if aa_subs_dicts[i] else []) for i,v in dp_dict['aa subs'].items()}
...
""" split AAS and PTM search for each sample with multiprocessing"""
p = mp.Pool(len(tissues))
data_dict_list = p.map(get_data_dict, tissues)
