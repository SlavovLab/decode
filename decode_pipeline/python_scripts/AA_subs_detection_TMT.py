...
"""
    This code reads in results from dependent peptide search with MaxQuant and dataset metadata (see example_sample_map.xlsx) from TMT-labeled data
    and outputs dictionaries of data for candidate peptides with amino acid substitutions (AAS).
    This script is computationally intensive to run and makes use of multiprocessing to split the search for AAS across samples (TMT sets). It is recommended to run in a HPC environment. 
...
# setting directories and reading in data. To be set by user. 
...
#MQ_TMT_dict = {'126':1,'127N':2, '127C':3, '128N':4, '128C':5, '129N':6, '129C':7, '130N':8, '130C':9, '131':10} # TMT channel:MaxQuant column label
...
path_to_fasta = aa_subs_dir+'GRCh38_latest_genomic.fna' #path to fasta file used for generating 6-frame genome translation.
